...
public class RowData extends ClusData implements MSortable, Serializable {
    ...
    public double[] getDataByFeatureDouble(int index) {
    	double [] featureVector = new double [m_Data.length];
    	for(int i = 0; i<m_Data.length; i++) {
    		featureVector[i] = m_Data[i].getDoubleVal(index);
    	}
    	return featureVector;
    }
    public int[] getDataByFeatureInteger(int index) {
    	int [] featureVector = new int [m_Data.length];
    	for(int i = 0; i<m_Data.length; i++) {
    		featureVector[i] = m_Data[i].getIntVal(index);
    	}
    	return featureVector;
    }
